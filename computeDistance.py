...
def get_distance_between_points(points, M, thresh_distance):
    actual_points = []
    all_points = []
    ...
    # returns set of all possible combinations of size 2 in the array transformed
    pairs = list(itertools.combinations(range(len(transformed)), 2))
    thresh_distance = 100

    for i, vec in enumerate(itertools.combinations(transformed, r=2)):
        actual_dist = math.sqrt((vec[0][0] - vec[1][0]) ** 2 + (vec[0][1] - vec[1][1]) ** 2)
        if actual_dist < thresh_distance:
            first = pairs[i][0]
            second = pairs[i][1]
