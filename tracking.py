...
class Tracker(Tracker):
    ...
    def __init__(self, xs, ys, sigma_h, sigma_iou, t_min, params_file, frameRate, metric='iou', emaAlpha=0.3,
                 update_rate=25):
    ...
    def update(self, bboxes, frame_num):
        ...
        for track in self.tracks_active:
            ...
            if len(bboxes) > 0:
                ...
                if self.metric == "iou":
                    best_match = max(bboxes,
                                     key=lambda bbox: iou(track['bboxes'][-1], bbox))  # x[0] is the bounding box
                    if iou(track['bboxes'][-1], best_match) >= self.sigma_iou:
                        flag = True
                else:
                    best_match = min(bboxes, key=lambda bbox: np.linalg.norm(
                        np.array(self.getLocation(track['bboxes'][-1])) - np.array(self.getLocation(bbox))))
                    flag = True

                if flag:
                    track['bboxes'].append(best_match)
                    track["bbDists"].append(
                        np.linalg.norm([best_match[1] - best_match[3], best_match[0] - best_match[2]]))
